...
public:
	...
	Graph_SearchIterDFS(const graph_type& graph,
		int          source,
		int          target = -1) :
...
//-----------------------------------------------------------------------------
template <class graph_type>
bool Graph_SearchIterDFS<graph_type>::Search()
{
...
//-----------------------------------------------------------------------------
template <class graph_type>
std::list<int> Graph_SearchIterDFS<graph_type>::GetPathToTarget()const
{
	std::list<int> path;
...
void Graph_SearchDijkstra<graph_type>::Search()
{
  ...
  pq.insert(m_iSource);//시작노드를 큐의 첫번째로 넣음
