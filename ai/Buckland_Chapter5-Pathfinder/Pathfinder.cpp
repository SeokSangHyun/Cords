...
//
//
void Pathfinder::CreatePathIterDFS()
{
	//set current algorithm
	m_CurrentAlgorithm = search_iterdfs;
	...
	//do the search
	Graph_SearchDFS<NavGraph> IterDFS(*m_pGraph, m_iSourceCell, m_iTargetCell);
	...
	if (IterDFS.Found()) { m_Path = IterDFS.GetPathToTarget(); }

	m_SubTree = IterDFS.GetSearchTree();
